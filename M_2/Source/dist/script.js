...
const drop = () => {
  //drag*
  //dragend*
  //dragenter - объект над dropArea (вошел в пределы)
  //dragexit*
  //dragleave - объект за пределами dropArea (покинул приделы)
  //dragover - объект двигается над dropArea
  //dragstart*
  //drop - объект отпущен над dropArea
  // * - срабатывают только при работе с элементами страницы
...
const modals = () => {
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', () => {
      ...
      if (!btnPressed && window.scrollY + document.documentElement.clientHeight + 1 >= scrollHeight // document.documentElement.scrollHeight вместо scrollHeight если не нужна поддержка старых браузеров
      ) {
...
const scrolling = upSelector => {
  ...
  let links = document.querySelectorAll('[href^="#"]'),
      speed = 0.5; // links = links.forEach((link) => link.href !== '#');
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', function () {
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_0__["getResource"])('./assets/db.json') // .then((res) => createCards(res))
    .then(res => createCards(res.styles)).catch(err => console.log(err));
    this.remove();
  });
  /* cards.forEach((card) => {
